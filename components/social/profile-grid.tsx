...
function XpProgression({ xp }: { xp: PublicXpSummary }) {
  ...
  return <div><p className="text-2xl font-bold text-[var(--profile-strong)]">Seviye {xp.level}</p><p className="text-sm text-[var(--app-text-secondary)]">{xp.selectedTitle ?? "Yolculuk devam ediyor"} · {xp.totalXp ?? 0} XP</p><div className="mt-3 grid grid-cols-3 gap-2 text-xs">{(Object.keys(worldLabels) as XpWorldKey[]).map((key)=>{const world=xp.worlds.find((item)=>item.key===key);return <div key={key} className="rounded-lg bg-[var(--app-surface-3)] p-2"><p className="font-medium">{worldLabels[key]}</p><p className="text-[var(--app-text-muted)]">Sv. {world?.level??1}</p></div>;})}</div><div className="mt-2 grid grid-cols-2 gap-2 text-[11px] sm:grid-cols-5">{(Object.keys(branchLabels) as XpBranchKey[]).map((key)=>{const branch=xp.branches.find((item)=>item.key===key);return <div key={key} className="rounded-lg border border-zinc-800 p-2"><p className="truncate text-[var(--app-text-secondary)]">{branchLabels[key]}</p><p className="text-[var(--app-text-muted)]">Sv. {branch?.level??1}</p></div>;})}</div>{xp.legacyImported&&<p className="mt-2 text-xs text-sky-300">Yerel ilerlemeden aktarım içerir.</p>}</div>;
}
  ...
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-12 md:auto-rows-min">
      {modules.map((module) => {
        ...
        return <section key={module.moduleKey} style={{ "--social-grid-row": `${module.gridY + 1} / span ${module.gridHeight}` } as CSSProperties} className={`social-profile-grid-item rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface-1)] p-4 ${SPANS[module.gridWidth]} ${STARTS[module.gridX]}`}>
          ...
          {module.moduleKey === "badges" && payload.xp && <div className="grid gap-2 sm:grid-cols-2">{payload.xp.badges.map((badge)=><div key={badge.key} className="rounded-xl bg-[var(--app-surface-3)] p-3"><p className="text-sm font-medium text-amber-200">{badge.name}</p><p className="mt-1 text-xs text-[var(--app-text-muted)]">{badge.description}</p></div>)}</div>}
