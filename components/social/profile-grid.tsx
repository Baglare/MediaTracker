    ...
    <div className="grid grid-cols-1 gap-4 md:grid-cols-12 md:auto-rows-min">
      {modules.map((module) => {
        ...
        return <section key={module.moduleKey} style={{ "--social-grid-row": `${module.gridY + 1} / span ${module.gridHeight}` } as CSSProperties} className={`social-profile-grid-item app-section rounded-2xl border border-[var(--app-border)] p-4 ${SPANS[module.gridWidth]} ${STARTS[module.gridX]}`}>
          ...
          {module.moduleKey === "shared_notes" && <div className="space-y-3">{payload.sharedNotes.map((note) => <article key={note.id} className="rounded-xl bg-[var(--app-surface-3)] p-3"><div className="flex justify-between gap-3"><p className="text-sm font-medium">{note.mediaTitle}</p>{note.containsSpoiler && <span className="text-xs text-[var(--app-warning)]">Spoiler</span>}</div>{note.containsSpoiler ? <details className="mt-2 text-sm text-[var(--app-text-muted)]"><summary className="cursor-pointer">Notu göster</summary><p className="mt-2 whitespace-pre-wrap">{note.content}</p></details> : <p className="mt-2 whitespace-pre-wrap text-sm text-[var(--app-text-muted)]">{note.content}</p>}</article>)}</div>}
