...
function XpProgression({ xp }: { xp: PublicXpSummary }) {
  ...
  return <div><p className="text-2xl font-bold text-violet-300">Seviye {xp.level}</p><p className="text-sm text-zinc-300">{xp.selectedTitle ?? "Yolculuk devam ediyor"} · {xp.totalXp ?? 0} XP</p><div className="mt-3 grid grid-cols-3 gap-2 text-xs">{(Object.keys(worldLabels) as XpWorldKey[]).map((key)=>{const world=xp.worlds.find((item)=>item.key===key);return <div key={key} className="rounded-lg bg-zinc-950/60 p-2"><p className="font-medium">{worldLabels[key]}</p><p className="text-zinc-500">Sv. {world?.level??1}</p></div>;})}</div><div className="mt-2 grid grid-cols-2 gap-2 text-[11px] sm:grid-cols-5">{(Object.keys(branchLabels) as XpBranchKey[]).map((key)=>{const branch=xp.branches.find((item)=>item.key===key);return <div key={key} className="rounded-lg border border-zinc-800 p-2"><p className="truncate text-zinc-300">{branchLabels[key]}</p><p className="text-zinc-500">Sv. {branch?.level??1}</p></div>;})}</div>{xp.legacyImported&&<p className="mt-2 text-xs text-sky-300">Yerel ilerlemeden aktarım içerir.</p>}</div>;
}
    ...
    <div className="grid grid-cols-1 gap-4 md:grid-cols-12 md:auto-rows-min">
      {modules.map((module) => {
        ...
        return <section key={module.moduleKey} style={{ "--social-grid-row": `${module.gridY + 1} / span ${module.gridHeight}` } as CSSProperties} className={`social-profile-grid-item rounded-2xl border border-zinc-800 bg-zinc-900/70 p-4 ${SPANS[module.gridWidth]} ${STARTS[module.gridX]}`}>
          ...
          {module.moduleKey === "progression" && !payload.xp && payload.progression && <div><p className="text-2xl font-bold text-violet-300">Seviye {payload.progression.level}</p><p className="text-sm text-zinc-300">{payload.progression.title} · {payload.progression.tier}</p><p className="mt-2 text-xs text-zinc-500">Yerel progression verisinden yayımlanan, doğrulanmamış snapshot.</p></div>}
          {module.moduleKey === "badges" && payload.xp && <div className="grid gap-2 sm:grid-cols-2">{payload.xp.badges.map((badge)=><div key={badge.key} className="rounded-xl bg-zinc-950/60 p-3"><p className="text-sm font-medium text-amber-200">{badge.name}</p><p className="mt-1 text-xs text-zinc-500">{badge.description}</p></div>)}</div>}
