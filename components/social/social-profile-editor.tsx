...
export default function SocialProfileEditor({ authConfigured, authenticated, userId, localPreferences, profileName, selectedTitle, media, progression, socialAvatarUrl, onProfileChanged }: { authConfigured: boolean; authenticated: boolean; userId: string | null; localPreferences: ProfilePreferences; profileName: string; selectedTitle: string; media: MediaItem[]; progression: UserProgression; socialAvatarUrl?: string; onProfileChanged: () => void | Promise<void> }) {
  ...
  async function saveProfile() {
    ...
    try { await post({ action: "save_profile", profile: validation.value }); setMessage("Sosyal profil kaydedildi."); await load(); await onProfileChanged(); }
  ...
  async function uploadBanner(file: File | undefined) {
    ...
    try { const response = await fetch("/api/social/assets", { method: "POST", body }); const result = await response.json() as { message?: string; cleanupPending?: boolean }; if (!response.ok) throw new Error(result.message); setMessage(result.cleanupPending ? "Banner güncellendi; eski dosya temizliği daha sonra gerekebilir." : "Banner güncellendi."); await load(); }
  ...
  return <div className="space-y-4">
    <section className="rounded-2xl border border-zinc-800 bg-zinc-900/40 p-4 sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div className="flex min-w-0 items-center gap-3">{data.profile && <ProfileAvatar profileName={profileName} preferences={localPreferences} socialAvatarUrl={socialAvatarUrl} size="sm" />}<div><h3 className="font-semibold">Cloud sosyal profil</h3><p className="mt-1 max-w-3xl text-xs text-zinc-500">Profil fotoğrafı yukarıdaki ortak kontrolden yönetilir. Yerel ad ve tagline yalnızca formu ön doldurur; yerel kütüphane/sync ayarları değişmez.</p></div></div>{data.profile?.username && <Link href={`/u/${data.profile.username}`} className="text-sm text-violet-300">Public profile git →</Link>}</div>
      ...
      <div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={() => void saveProfile()} className="rounded-lg bg-violet-600 px-4 py-2 text-sm font-medium text-white">Sosyal profili kaydet</button>{data.profile && <label className="cursor-pointer rounded-lg border border-zinc-700 px-3 py-2 text-sm">Banner yükle<input type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" onChange={(event) => void uploadBanner(event.target.files?.[0])} /></label>}</div>{data.profile?.usernameChangedAt && <p className="mt-2 text-xs text-zinc-500">Son username kaydı: {new Date(data.profile.usernameChangedAt).toLocaleDateString("tr-TR")}. Sonraki değişiklik RPC’de 30 günlük kurala tabidir.</p>}{message && <p role="status" className="mt-3 text-sm text-zinc-400">{message}</p>}
    </section>
    {data.profile ? <>{userId && <SocialPreferencesPanel userId={userId} />}<SocialLayoutEditor modules={modules} profileVisibility={form.visibilityMode} onChange={setModules} onSave={async () => { try { await post({ action: "save_modules", modules }); setMessage("Profil düzeni kaydedildi."); } catch (error) { setMessage(error instanceof Error ? error.message : "Düzen kaydedilemedi."); } }} /><SocialSharingEditor media={media} progression={progression} favorites={data.favorites} current={data.current} sharedNotes={data.sharedNotes} onRefresh={load} />{data.blockedAccounts.length > 0 && <section className="rounded-2xl border border-zinc-800 bg-zinc-900/40 p-4 sm:p-5"><h3 className="font-semibold">Engellenen hesaplar</h3><div className="mt-3 space-y-2">{data.blockedAccounts.map((account) => <div key={account.id} className="flex items-center justify-between rounded-lg bg-zinc-950/60 px-3 py-2 text-sm"><span>{account.displayName}{account.username ? ` · @${account.username}` : ""}</span><button type="button" onClick={() => void unblock(account.id)} className="text-xs text-violet-300">Engeli kaldır</button></div>)}</div></section>}</> : <section className="rounded-2xl border border-dashed border-zinc-800 p-5 text-sm text-zinc-500">Grid, vitrin ve paylaşım ayarları ilk sosyal profil kaydından sonra açılır.</section>}
  </div>;
}
