...
`20260809120000_d8_public_profile_theme.sql` additive migration’dır. `profile_theme_visibility` varsayılanı `hidden` olur, owner-only save RPC yeni alanları yazar ve public RPC yalnız yayımlanmış snapshot’ı döndürür. Doğrudan RPC çağrısı da tam root/token allowlist’i, hex biçimi, preset-revision eşleşmesi ve aynı kritik kontrast eşikleriyle fail-closed doğrulanır. Bu belge D8-2 tamamlandığı andaki durumu anlatıyordu; migration ve staging RLS kabulü daha sonra D8-4A'da geçti. Production rollout hâlâ ayrı D8-4B kapısıdır.
