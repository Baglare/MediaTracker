...
| TMDB | Film arama/takvim | [FAQ](https://developer.themoviedb.org/docs/faq): non-commercial attribution ile; commercial lisans ayrı. About/Credits'te TMDB notice ve resmi logo gerekir. | Mode/token tek başına yetmez; approved logo asset repo'da olmadığından capability `attribution_required` ile kapalı. |
...
- AniList public Production permission kararı kapanmadan `authorized` kullanılmaz.
- OMDb public release yolu kapatılmıştır; yeniden açma ayrı lisans kararı gerektirir.
- Resmi TMDB logo/brand asset'i ve dağıtım license-notice yüzeyi tamamlanmalı.
- Open Library Production UA gerçek operator contact bilgisiyle manuel doğrulanmalı.
