...
declare v_profile_visibility text;
begin
  ...
  select visibility_mode into v_profile_visibility from public.profiles where id=p_owner and deleted_at is null;
  if v_profile_visibility is null or v_profile_visibility = 'personal' then return false; end if;
  if v_profile_visibility = 'public' and p_visibility = 'public' then return true; end if;
  ...
  if p_visibility in ('public', 'followers') then return true; end if;
