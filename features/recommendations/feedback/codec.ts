...
const MAX_FEEDBACK_ASPECT_IDS = 12;
const MAX_FEEDBACK_CONSTRAINT_KEYS = 24;
...
function rejectUnknown(value: Record<string, unknown>, allowed: readonly string[], path: string): RecommendationDomainIssue[] {
  const fields = new Set(allowed);
  return Object.keys(value).filter((key) => !fields.has(key)).map((key) => (
    issue("feedback_unknown_field", `${path}.${key}`, "Feedback V2 bilinmeyen alan kabul etmez.")
  ));
}
...
export function decodeRecommendationFeedbackEventV2(value: unknown): RecommendationDecodeResult<RecommendationFeedbackEventV2> {
  ...
  const issues: RecommendationDomainIssue[] = rejectUnknown(value, [
    "version", "id", "action", "candidateIdentity", "requestId", "sessionId", "resultKind",
    "reasonCode", "aspectIds", "constraintKeys", "createdAt", "metadata",
  ], "$" );
  ...
  issues.push(...rejectUnknown(identity, identity.kind === "provider"
    ? ["kind", "provider", "externalId", "mediaType"]
    : ["kind", "libraryItemId", "mediaType"], "candidateIdentity"));
  ...
  if (Array.isArray(value.aspectIds) && value.aspectIds.length > MAX_FEEDBACK_ASPECT_IDS) issues.push(issue("feedback_aspects_limit_exceeded", "aspectIds", "Feedback aspect sınırı aşıldı."));
  ...
  if (Array.isArray(value.constraintKeys) && (value.constraintKeys.length > MAX_FEEDBACK_CONSTRAINT_KEYS || constraintKeys.length !== value.constraintKeys.length)) issues.push(issue("feedback_constraints_limit_exceeded", "constraintKeys", "Feedback constraint sınırı veya değeri geçersiz."));
  if (value.requestId !== undefined && !text(value.requestId, 128)) issues.push(issue("feedback_request_id_invalid", "requestId", "requestId geçersiz."));
  if (value.sessionId !== undefined && !text(value.sessionId, 128)) issues.push(issue("feedback_session_id_invalid", "sessionId", "sessionId geçersiz."));
  if (value.metadata !== undefined) {
    if (!record(value.metadata)) issues.push(issue("feedback_metadata_invalid", "metadata", "Metadata nesne olmalıdır."));
    else issues.push(...rejectUnknown(value.metadata, ["fitLabel", "objectiveField"], "metadata"));
  }
