...
export function AspectConstraintEditor({ constraints, capabilities = [], onChange }: { constraints: readonly AspectConstraint[]; capabilities?: readonly ConstraintEvidenceCapability[]; onChange: (next: AspectConstraint[]) => void }) {
  ...
  function update(index: number, role: AspectConstraint["role"]) {
    ...
    const capability = capabilities.find((item) => item.constraintId === current.id);
    if ((role === "must" && capability && !capability.canUseAsMust)
      || (role === "avoid" && capability && !capability.canUseAsAvoid)) return;
  ...
  return <div className="space-y-2">
    ...
    {constraints.map((constraint, index) => {
      ...
      const capability = capabilities.find((item) => item.constraintId === constraint.id);
      return <div key={constraint.id} className="flex flex-wrap items-center gap-2 rounded-lg border border-zinc-800/70 px-2 py-2">
        ...
        <select aria-label={`${entry.labelTr} rolü`} value={constraint.role} onChange={(event) => update(index, event.target.value as AspectConstraint["role"])} className="rounded border border-zinc-700 bg-zinc-900 px-2 py-1 text-xs">
          <option value="must" disabled={capability ? !capability.canUseAsMust : String(entry.mustSafety) === "unsafe"}>{ROLE_LABEL.must}</option>
          <option value="prefer">{ROLE_LABEL.prefer}</option>
          <option value="avoid" disabled={capability ? !capability.canUseAsAvoid : String(entry.avoidSafety) === "unsafe"}>{ROLE_LABEL.avoid}</option>
        </select>
        ...
        {capability && <p className={`w-full text-[10px] ${constraint.role === "prefer" || (constraint.role === "must" ? capability.canUseAsMust : capability.canUseAsAvoid) ? "text-zinc-500" : "text-amber-300/80"}`}>{capability.userMessage}</p>}
