    ...
    {constraints.filter((constraint) => constraint.field !== "media_type").map((constraint) => {
      ...
      return <div key={constraint.id} className="flex flex-wrap items-center gap-2 rounded-lg border border-zinc-800/70 px-2 py-2">
        ...
        {(constraint.field === "length" || constraint.field === "release_year") && <>
          <select aria-label={`${constraint.field} operatörü`} value={constraint.operator} onChange={(event) => { const operator = event.target.value as NumericConstraintOperator; setConstraint(index, operator === "between" ? { ...constraint, operator, value: undefined, min: 0, max: constraint.field === "release_year" ? 2030 : 12 } : { ...constraint, operator, value: constraint.value ?? constraint.max ?? 0, min: undefined, max: undefined }); }} className="rounded border border-zinc-700 bg-zinc-900 px-2 py-1 text-xs">{Object.entries(OPERATOR_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>
          {constraint.operator === "between" ? <><input aria-label={`${constraint.field} minimum`} type="number" min={0} value={constraint.min ?? ""} onChange={(event) => setConstraint(index, { ...constraint, min: Number(event.target.value) })} className="w-20 rounded border border-zinc-700 bg-zinc-900 px-2 py-1 text-xs" /><input aria-label={`${constraint.field} maximum`} type="number" min={0} value={constraint.max ?? ""} onChange={(event) => setConstraint(index, { ...constraint, max: Number(event.target.value) })} className="w-20 rounded border border-zinc-700 bg-zinc-900 px-2 py-1 text-xs" /></> : <input aria-label={`${constraint.field} değeri`} type="number" min={0} value={constraint.value ?? ""} onChange={(event) => setConstraint(index, { ...constraint, value: Number(event.target.value) })} className="w-24 rounded border border-zinc-700 bg-zinc-900 px-2 py-1 text-xs" />}
          {constraint.field === "length" && <span className="text-[10px] text-zinc-500">{constraint.unit}</span>}
        </>}
        {constraint.field !== "length" && constraint.field !== "release_year" && <input aria-label={`${constraint.field} değeri`} value={constraint.value} onChange={(event) => setConstraint(index, { ...constraint, value: event.target.value })} className="rounded border border-zinc-700 bg-zinc-900 px-2 py-1 text-xs" />}
        <button type="button" aria-label={`${constraint.field} koşulunu kaldır`} onClick={() => onChange(constraints.filter((_, i) => i !== index))} className="ml-auto text-xs text-zinc-500 hover:text-zinc-200">Kaldır</button>
      </div>;
    })}
