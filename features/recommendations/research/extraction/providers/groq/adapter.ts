...
export class GroqGroundedExtractionAdapter implements GroundedExtractionProviderPort {
  ...
  async extract(input: Parameters<GroundedExtractionProviderPort["extract"]>[0]) {
    ...
    } catch (error) { const kind = error instanceof ExtractionHttpError ? error.kind : "network"; const captured = error instanceof ExtractionHttpError ? error.telemetry : undefined; return { status: kind === "aborted" || kind === "timeout" ? "budget_exhausted" as const : kind === "rate_limited" ? "rate_limited" as const : "provider_unavailable" as const, providerId: this.providerId, modelId: input.model, telemetry: { durationMs: captured?.durationMs ?? 0, retryCount: captured?.retryCount ?? 0, rateLimitCount: captured?.rateLimitCount ?? (error instanceof ExtractionHttpError && error.status === 429 ? 1 : 0), responseBytes: 0, ...(captured?.rateLimit ? { rateLimit: captured.rateLimit } : {}) }, warnings: [`groq_extraction_${kind}`] }; }
  }
}
