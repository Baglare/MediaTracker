...
import { deriveGroundedAssessmentPassages, summarizeGroundingFailure } from "../domain/model-output";
...
import { buildGroundedEvidenceWorkingSet } from "../prompt/working-set";
...
async function execute(value: unknown, dependencies: GroundedExtractionDependencies): Promise<GroundedExtractionResult> {
  ...
  const workingSet = buildGroundedEvidenceWorkingSet({ packet: request.packet, eligibleUnits: units.eligibleUnits, maxUnits: request.maxEvidenceUnits });
  telemetry.sentEvidenceUnitCount = workingSet.sentUnitCount; telemetry.packetEvidenceCharacters = workingSet.packetCharacters; telemetry.sentEvidenceCharacters = workingSet.sentCharacters; telemetry.lexicalUnitsRetained = workingSet.lexicalUnitsRetained; telemetry.contextUnitsRetained = workingSet.contextUnitsRetained;
  if (workingSet.units.length === 0) {
    const aggregated = await aggregateGroundedExtraction({ request, output: { version: 1, assessments: [] }, units: [], now: dependencies.now });
    return { status: "no_claims_extracted", assessments: [], claims: [], decision: aggregated.decision, telemetry, warnings: ["grounded_working_set_insufficient"] };
  }
  ...
  const modelInput = buildMinimizedGroundedModelInput({ request, units: workingSet.units });
  ...
  const canonicalOutput = deriveGroundedAssessmentPassages({ output: adapterResult.output, units: workingSet.units });
  const grounded = validateProviderGroundedObservation({ request, output: canonicalOutput, units: workingSet.units, excludedUnitIds: units.excludedUnitIds });
  if (!grounded.ok) {
    const summary = summarizeGroundingFailure({ issues: grounded.issues, output: adapterResult.output });
    telemetry.groundingFailureCode = summary.failureCode; telemetry.groundingAssessmentCount = summary.assessmentCount; telemetry.groundingReferencedUnitCount = summary.referencedUnitCount;
    return failure("grounding_invalid", [`grounding_failure:${summary.failureCode}`], telemetry, providerId, adapterResult.modelId);
  }
  telemetry.assessmentCount = grounded.value.assessments.length;
  const aggregated = await aggregateGroundedExtraction({ request, output: grounded.value, units: workingSet.units, now: dependencies.now }); telemetry.claimCount = aggregated.claims.length;
  const completedAt = (dependencies.now ?? (() => new Date()))();
  const provenance = { providerId, modelId: adapterResult.modelId, schemaVersion: request.schemaVersion, extractorPolicyVersion: request.extractorPolicyVersion, packetContentHash: request.packet.packetContentHash, extractionStartedAt: startedAt.toISOString(), extractionCompletedAt: completedAt.toISOString(), assessmentCount: grounded.value.assessments.length, validEvidenceUnitCount: workingSet.units.length, responseStatus: aggregated.claims.length > 0 ? "validated" as const : "no_claims" as const, warnings: adapterResult.warnings };
