...
import { resolvePinnedResearchAddress } from "@/features/recommendations/research/network/dns-policy";
...
import { SecureResearchHttpError, type SecureResearchHttpRequest } from "@/features/recommendations/research/network/types";
...
describe("D7-R2A IP/DNS policy", () => {
  ...
  it("mixed public/private DNS sonucunu fail-closed reddeder", async () => {
    await expect(resolvePinnedResearchAddress({ hostname: "www.wikidata.org", resolver: new FakeResearchDnsResolver([[...PUBLIC, { address: "10.0.0.1", family: 4 }]]) })).rejects.toMatchObject({ kind: "security_rejected" });
  });

  it("boş DNS ve bounded timeout ayrımını korur", async () => {
    await expect(resolvePinnedResearchAddress({ hostname: "www.wikidata.org", resolver: new FakeResearchDnsResolver([[]]) })).rejects.toMatchObject({ kind: "dns_unavailable" });
...
describe("D7-R2A pinned HTTPS, redirect ve retry", () => {
  ...
  it("redirect yeni DNS validation alır ve private hedef çözümünü reddeder", async () => {
    ...
    await expect(new SecureResearchHttpClientImpl(resolver, transport).request(baseRequest())).rejects.toMatchObject({ kind: "security_rejected" });
  ...
  it("transient network/timeout retry olur; security error retry olmaz", async () => {
    const transport = new FakeResearchHttpsTransport([new SecureResearchHttpError("network", "temporary", true), fakeTransportJson()]);
