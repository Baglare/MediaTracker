...
export default async function RecommendationsPage({searchParams}:{searchParams:Promise<{to?:string;compose?:string}>}){const [auth,params]=await Promise.all([getCurrentServerAuth(),searchParams]);const recipient=params.to?validateUuid(params.to):null;return !auth.configured?<SocialSignInRequired configured={false}/>:!auth.userId?<SocialSignInRequired/>:<SocialPageShell eyebrow="Sosyal" title="Medya Önerileri" subtitle="Gelen ve gönderilen önerileri cevap ve ilerleme durumlarıyla yönet." icon={<Send className="h-5 w-5" aria-hidden="true"/>}><RecommendationInbox userId={auth.userId} initialRecipientId={recipient?.ok?recipient.value:undefined} composeInitially={params.compose==="1"}/></SocialPageShell>;}
